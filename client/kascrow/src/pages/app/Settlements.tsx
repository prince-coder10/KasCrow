...
import { useSettlements } from "./resources/Settlements.resource";
...
function SettlementsList() {
  ...
  return (
    <>
      {settlements.map((settlement: IEscrow) => {
        const config = getStatusConfig(settlement.status);
        const StatusIcon = config.icon;

        return (
          <div
            key={settlement.escrowId}
            ...
            onClick={() =>
              navigate(`/settlement/${settlement.escrowId}/create`)
            }
          >
            {/* first child */}
            <div className="w-full flex gap-4 items-center ">
              ...
              <div className="flex flex-col">
                <p className="text-[15px] font-medium">{settlement.title}</p>
    ...
    >
      <div className="w-full mt-5 flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <p className="text-[24px] font-semibold">Settlements</p>
        </div>
